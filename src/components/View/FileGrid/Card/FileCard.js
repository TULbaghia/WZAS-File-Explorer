    ...
    if (!results.length) {
      if (isAnyAudioOpened(getFileHandle) && handle.name.includes(".mp3")) {
        addToAudioList({
          ...
          handle,
          getCurrentDir,
          directoryStack: [...getDirectoryStack],
        });
        dispatchAlert({
          title: "Sukces",
          message: "Plik został dodany do kolejki odtwarzania",
        });
      } else {
        dispatchPushFileHandle({
          name,
          handle,
          getCurrentDir,
          directoryStack: [...getDirectoryStack],
      ...
      dispatchAlert({
        ...
        message: (
          <>
            ...
            <br />
            <small style={colorCrimson}>{name}</small>
          </>
        ),
        showCancel: true,
        callbackOnOk: () =>
          removeFile(getCurrentDir(), handle, dispatchProgress),
      });
    } else {
      dispatchAlert({
        title: "Błąd",
      ...
      dispatchPrompt({
        ...
        message: (
          <>
            ...
            <br />
            <small>
              Zmiana nazwy polega na kopiowaniu, może to chwilę potrwać
            </small>
          </>
        ),
        label: "Nazwa pliku",
        callbackOnOk: (newName) =>
          moveFile(
            getCurrentDir(),
            name,
            getCurrentDir(),
